...
// Framebuffer: store RGB565 in big-endian inside uint16_t so that
// (uint8_t*)g_fb is ready for MSB-first SPI streaming.
static uint16_t g_fb[GC9A01A_W * GC9A01A_H];

// ===== Pin helpers from CubeMX main.h =====
...
static inline void BLK_L(void) { HAL_GPIO_WritePin(TFT_BLK_GPIO_Port, TFT_BLK_Pin, GPIO_PIN_RESET); }
static inline void BLK_H(void) { HAL_GPIO_WritePin(TFT_BLK_GPIO_Port, TFT_BLK_Pin, GPIO_PIN_SET); }
...
uint16_t GC9A01A_Simple_GetWidth(void)  { return g_w; }
uint16_t GC9A01A_Simple_GetHeight(void) { return g_h; }

void GC9A01A_Simple_SetBL(uint8_t percent)
{
    if (percent > 100) percent = 100;

    uint32_t arr = __HAL_TIM_GET_AUTORELOAD(&htim11);   // например 4999
    uint32_t ccr = (arr * percent) / 100u;              // 0..ARR
...
{
    GC9A01A_Simple_FillRect(0, 0, (int16_t)g_w, (int16_t)g_h, color);
}

// ===== DMA Update (chunked, callback chain) =====
...
static void tx_next(void)
{
    ...
    SPI_send_dma(SPI1, 0, p, chunk, tx_next);
}
